...
// 1. Patch RuntimeScheduler.h (C++ constructor annotation for Swift 6.2+)
...
if (fs.existsSync(jsRuntimeTarget)) {
  ...
  // Ensure regex identifier compatibility
...
// 3. Patch build-xcframework.sh to pass SWIFT_STRICT_CONCURRENCY=off and SWIFT_ENFORCE_EXCLUSIVE_ACCESS=none
...
// 4. Patch KeepAwakeModule.swift if present for Sendable conformance
const keepAwakePaths = [
  path.join(rootDir, 'node_modules', 'expo', 'node_modules', 'expo-keep-awake', 'ios', 'KeepAwakeModule.swift'),
  path.join(rootDir, 'node_modules', 'expo-keep-awake', 'ios', 'KeepAwakeModule.swift'),
];

keepAwakePaths.forEach((keepAwakePath) => {
  if (fs.existsSync(keepAwakePath)) {
    let content = fs.readFileSync(keepAwakePath, 'utf8');
    if (content.includes('public final class KeepAwakeModule: Module {')) {
      content = content.replace(
        'public final class KeepAwakeModule: Module {',
        'public final class KeepAwakeModule: Module, @unchecked Sendable {'
      );
      fs.writeFileSync(keepAwakePath, content, 'utf8');
      console.log(`✓ Added @unchecked Sendable to ${keepAwakePath}`);
    }
  }
});
